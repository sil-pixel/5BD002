...
generate_data <- function(n, dag) {
  ...
  M <- rbinom(n, 1, 0.5)
  S <- rbinom(n, 1, 0.5)
  # probs for "zero" and then "one-vs-two" among the nonzeros
  p0 <- pX(0.5*M, 0.5*S)   # P(X=0 | M,S)
  q1 <- pX(0.5*M, 0.5*S)   # P(X=1 | X>0, M,S)
  # draw X from {0,1,2}
  is0 <- rbinom(n, 1, p0)                     # 1 -> X=0
  is1 <- rbinom(n, 1, q1)                     # among nonzeros: 1 -> X=1, else 2
  X   <- ifelse(is0 == 1, 0, ifelse(is1 == 1, 1, 2))
  ...
  dat$X_num <- as.numeric(X)               # 0/1/2 for Y-model
  dat$X_ord <- ordered(X, levels = 0:2)    # ordered factor for ind-model
  return(dat)
}
...
fit_frm_em_once <- function(n, dag) {
  ...
  dep <- list(model="linreg", formula=Y ~ X_num + M + S)
  ind <- list(X = list(model="oprobit", formula=X_ord ~ M + S))
...
The assignment took 17 hours to complete.